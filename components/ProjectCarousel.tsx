...
export default function ProjectCarousel({ projects }: { projects: any[] }) {
    ...
    return (
        <div className="col-span-1 md:col-span-2 lg:col-span-4 rounded-2xl p-6 md:p-8 border border-glass-border bg-glass-bg backdrop-blur-md flex flex-col gap-6 relative overflow-hidden">
            ...
            {/* Carousel Container */}
            <div ref={carouselRef} className="overflow-hidden z-10 relative pb-4 pt-2 -mx-4 px-4 md:-mx-8 md:px-8 cursor-grab active:cursor-grabbing">
                <motion.div 
                    ref={innerRef}
                    drag="x"
                    ...
                    onDragEnd={(e, info) => {
                        ...
                        setCurrentX(Math.max(-carouselWidth, Math.min(0, currentX + info.offset.x)));
                    }}
                ...
                >
                    ...
                    {projects.length > 0 ? projects.map((project: any) => (
                        ...
                        <div className="text-slate-500 text-sm p-4 min-w-[300px]">No projects added yet from admin.</div>
                    )}
                </motion.div>
            </div>
